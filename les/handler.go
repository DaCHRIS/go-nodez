...
// Package les implements the Light Nodez Subprotocol.
package les
...
import (
	...
	"github.com/NodezCrypto/go-nodez/light"
	...
	"github.com/NodezCrypto/go-nodez/trie"
)
...
// NewProtocolManager returns a new ethereum sub protocol manager. The Nodez sub protocol manages peers capable
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Nodez protocol")
	...
	log.Info("Light Nodez protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Nodez peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Nodez message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Nodez message arrived", "code", msg.Code, "bytes", msg.Size)
...
// NodeInfo represents a short summary of the Nodez sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Nodez network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
