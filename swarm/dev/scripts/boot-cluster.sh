...
#
# The cluster contains a bootnode, a ndz node and multiple swarm nodes, with
...
# static ndz configuration
NODEZ_IP="192.168.33.3"
NODEZ_RPC_PORT="7331"
NODEZ_RPC_URL="http://${NODEZ_IP}:${NODEZ_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the ndz and
# swarm nodes
start_bootnode() {
...
# start_NODEZ_node starts a ndz node with --datadir pointing at <base-dir>/ndz
# and a single, unlocked account with password "ndz"
start_NODEZ_node() {
  local dir="${base_dir}/ndz"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get ndz account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${NODEZ_IP}"
    --rpcport   "${NODEZ_RPC_PORT}"
    --verbosity "6"
  )

  start_node "ndz" "${NODEZ_IP}" "$(which ndz)" ${args[@]}
}
...
create_account() {
  ...
  ndz --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
