...
// Package ndz contains the simplified mobile APIs to go-nodez.
//
// The scope of this package is *not* to allow writing a custom Nodez client
// with pieces plucked from go-nodez, rather to allow writing native dapps on
...
// from the go-nodez repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-nodez version.
