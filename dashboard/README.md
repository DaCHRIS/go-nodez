...
The dashboard is a data visualizer integrated into ndz, intended to collect and visualize useful information of an Nodez node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-nodez` repo), these need to be installed first:
...
Normally the dashboard assets are bundled into Nodez via `go-bindata` to avoid external dependencies. Rebuilding Nodez after each UI modification however is not feasible from a developer perspective. Instead, we can run `webpack` in watch mode to automatically rebundle the UI, and ask `ndz` to use external assets to not rely on compiled resources:

```
$ (cd dashboard/assets && ./node_modules/.bin/webpack --watch)
$ ndz --dashboard --dashboard.assets=dashboard/assets/public --vmodule=dashboard=5
```

To bundle up the final UI into Nodez, run `go generate`:
