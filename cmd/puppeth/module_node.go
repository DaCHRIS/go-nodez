...
// nodeDockerfile is the Dockerfile required to run an Nodez node.
var nodeDockerfile = `
...
// an Nodez node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    volumes:
      - {{.Datadir}}:/root/.nodez{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.ndzhash{{end}}
    environment:
...
// deployNode deploys a new Nodez node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 ndz --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.nodez"],
		ethashdir:  infos.volumes["/root/.ndzhash"],
