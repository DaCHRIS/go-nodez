...
func TestAccountListEmpty(t *testing.T) {
	ndz := runNodez(t, "account", "list")
	ndz.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	ndz := runNodez(t, "account", "list", "--datadir", datadir)
	defer ndz.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	ndz := runNodez(t, "account", "new", "--lightkdf")
	defer ndz.ExpectExit()
	ndz.Expect(`
...
`)
	ndz.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	ndz := runNodez(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer ndz.ExpectExit()
	ndz.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(ndz.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(ndz.StderrText(), m) {
...
// https://github.com/NodezCrypto/go-nodez/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	ndz.SetTemplateFunc("keypath", func(file string) string {
